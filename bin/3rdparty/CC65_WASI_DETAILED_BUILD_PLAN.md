...
### A5. Configure cc65 search paths

Prefer stable WASI guest paths as compile-time defaults:
...
Environment variables may still override these values where cc65 supports them, but the normal launcher should not depend on empty environment variables to disable unusable native paths.

Example additional compiler definitions:
...
The exact quoting must be tested against the cc65 Makefile. If command-line quoting becomes fragile, place these definitions in a small generated header and inject it with `-include`.

### A6. Record reproducibility behavior
...
make -C "$CC65_ROOT/src" \
    ...
    RANLIB="$LLVM_RANLIB" \
    EXE_SUFFIX=".wasm" \
    USER_CFLAGS="--target=$WASI_TARGET --sysroot=$WASI_SYSROOT -O3 $COMMON_DEFINES" \
...
The first WASI build must confirm how cc65 v2.19 handles these Make variables:
...
If the Makefile replaces rather than appends a variable, adjust the invocation without modifying upstream rules unnecessarily. Do not fall back to Emscripten merely because a variable requires different quoting or placement.
...
8. Patch only verified portability problems; do not add generic syscall emulation.
...
|------|--------|------------|
| A cc65 source file uses an API unavailable in WASI Preview 1 | Compile or runtime failure | Identify the exact call; use a small portable source change or feature flag |
| Incorrect Make variable quoting | Wrong defines or failed build | Verify compiler command lines; use a generated header if necessary |
