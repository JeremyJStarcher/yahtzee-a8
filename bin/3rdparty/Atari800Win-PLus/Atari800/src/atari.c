...
#ifdef WASI_EMBED_ROMS
#include "atari800_roms.h"
#endif /* WASI_EMBED_ROMS */

int Atari800_LoadImage(const char *filename, UBYTE *buffer, int nbytes)
{
#ifdef WASI_EMBED_ROMS
	/* The WASI build cannot open files (emcc STANDALONE_WASM has no
	   path_open), so serve the known ROM images from arrays compiled
	   into the module by gen_roms.py. */
	{
		const UBYTE *rom_src = NULL;
		int rom_size = 0;
		if (strstr(filename, "ATARIXL") != NULL) {
			rom_src = g_rom_atarixl;
			rom_size = sizeof(g_rom_atarixl);
		}
		else if (strstr(filename, "ATARIBAS") != NULL) {
			rom_src = g_rom_ataribas;
			rom_size = sizeof(g_rom_ataribas);
		}
		else if (strstr(filename, "ATARIOSB") != NULL) {
			rom_src = g_rom_atariosb;
			rom_size = sizeof(g_rom_atariosb);
		}
		if (rom_src != NULL) {
			if (rom_size != nbytes) {
				Log_print("Embedded ROM size mismatch: %s", filename);
				return FALSE;
			}
			memcpy(buffer, rom_src, nbytes);
			return TRUE;
		}
	}
#endif /* WASI_EMBED_ROMS */
